...
if __name__ == '__main__':
    ...
    #     # 捕获异常并输出错误信息，然后继续处理下一个模型
    #     print(f"模型 {seed_model} MR：{MR} 发生错误：{e}")
    #     print("跳过该模型，继续下一个模型。")
    # finally:
    #     eend = time.time()
    #     end_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    #     elapsed_time = eend - sstart
    #     print('~~~~~~~~~~~~~~~~~~~~~~~~')
    #     print(f"模型 {seed_model} MR：{MR} 开始时间：{start_time}")
    #     print(f"模型 {seed_model} MR：{MR} 结束时间：{end_time}")
    #     print(f"模型 {seed_model} MR：{MR} 用时：{elapsed_time:.2f} 秒")
    #     print('~~~~~~~~~~~~~~~~~~~~~~~~~END~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
