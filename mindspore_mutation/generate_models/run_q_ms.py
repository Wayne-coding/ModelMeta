...
from mindspore_mutation.cargo import get_loss
import gc
...
def run_q_ms(seed_model, mutate_times, num_samples, mr_index):
    ...
    Quantum_Q = QRDQN(state_dim, n_actions).to(pt_device)
    Target_Q = copy.deepcopy(Quantum_Q).to(pt_device)
    Target_Q.eval()  
    ...
    for n in range(mutate_times):
        ...
        try:
            ...
            if api_mutation_type == 'seed_model' and False:
                try:
            ...
            loss_ms = loss_fun_ms(new_output, original_outputs)
        ...
        with torch.no_grad():
            ...
            formatted_data_torch_nograd = torch.from_numpy(formatted_data.asnumpy()).to(pt_device)
            next_q_value=Target_Q(formatted_data_torch_nograd).max(1)[0]
        ...
        df = pd.DataFrame([(k, v[0], v[1],v[2],v[3],v[4],v[5],v[6]) for k, v in metrics_dict.items()],
                        columns=['New_Model_Name', 'Input_cov','Config_cov','Api_cov','Avg_cov','Elapsed_time', 'Gpu_Memory_Used', 'Cpu_Memory_Used'])
    ...
    for index, (name, new_net) in enumerate(N.items()):
        ...
        with open('/data1/czx/SemTest_master/mindspore_mutation/results/example.txt', 'a', encoding='utf-8') as file:
            ...
            if isinstance(n, (str, int)):
                file.write("层数：" + str(n) + '-' + str(len(list111)) + '\n')  
            else:
                
                file.write('\n'.join(map(str, n)) + '\n')
            
            file.write(', '.join(list111) + ',')  
        try:
            input_cov, config_cov, api_cov, op_type_cov, op_num_cov, edge_cov, inside, output_datas = model2cov(new_net,
                                                                                                    input_data,
                                                                                                    dtypes,
            ...
            if 'input_cov' in dir():
                sheet.cell(row=counter, column=1).value = INDEX_N[index-1]
