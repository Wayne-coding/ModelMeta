...
def run_q_ms(seed_model, mutate_times, num_samples, mr_index, ifapimut, num_quantiles,ifeplison, ifTompson,device,train_config,csv_file_path):
    ...
    for n in range(mutate_times):
        ...
        with torch.no_grad():
            if ('state' in log_dict[n]) and ("Failed" in log_dict[n]['state']):  # 在上述过程中变异失败了
                ...
                # print("cov_input_data: ",input_data[0].shape)
                # print(new_net(input_data[0]))
                # print(mindsporeinfoplus.summary_plus(
                #         model=new_net,
    ...
    #     list111 = []
    #     # 遍历 node_list 并将 name1 添加到 list111
...
if __name__ == '__main__':
    ...
    if seed_model == "openpose" or "SSD" in seed_model:
        ...
        label_selected1 = Tensor.from_numpy(samples_label1)
        label_selected2 = Tensor.from_numpy(samples_label2)
        label_selected3 = Tensor.from_numpy(samples_label3)
        label_selected4 = Tensor.from_numpy(samples_label4)
        label_selected5 = Tensor.from_numpy(samples_label5)
        label_selected6 = Tensor.from_numpy(samples_label6)
    ...
    if seed_model in ["ssimae", "crnn", "openpose"]:
        pass
    elif seed_model =="patchcore":
        from models.PatchCore.src.oneStep import OneStepCell
        seed_model_net = OneStepCell(seed_model_net)
        ...
        loss_fun_ms, _ = get_loss(lossname)
        loss_fun_ms = loss_fun_ms()


    seed_optimizer_fun = seed_optimizer_ms(params=seed_model_net.trainable_params(), learning_rate=1e-4)
    ...
    if seed_model == "patchcore":
        ...
        if seed_model == "ssimae":
            ...
            loss = SSIMLoss()
            train_net_ms = NetWithLoss(seed_model_net, loss)
            loss,grads = train_step_ssimae(train_net_ms, seed_optimizer_fun, data_selected)
            ...
            crnnloss = CTCLoss(max_sequence_length=crnnconfig.num_step,
                               max_label_length=crnnconfig.max_text_length,
                               batch_size=num_samples)
            train_net_ms = WithLossCell(seed_model_net, crnnloss)
            loss,grads = train_step_crnn(train_net_ms, seed_optimizer_fun, data_selected, label_selected)
            ...
            criterion = openpose_loss_ms()
            train_net_ms = BuildTrainNetwork_ms(seed_model_net, criterion)
            loss,grads = train_step_openpose(train_net_ms, seed_optimizer_fun, data_selected, label_selected1, label_selected2, label_selected3)
    elif "SSD" in seed_model:
        loss, grads = train_step_ssd(seed_model_net,loss_fun_ms, seed_optimizer_fun, data_selected, label_selected1, label_selected2, label_selected3)
        ...
        yolo_output = seed_model_net(data_selected)
        loss = loss_fun_ms(yolo_out=yolo_output, y_true_0= label_selected1,
                                         y_true_1=label_selected2,
                                         y_true_2=label_selected3,
                                         gt_0=label_selected4, gt_1=label_selected5, gt_2=label_selected6,shape=input_shape_ms)
        # def forward_fn(images, batch_y_true_0, batch_y_true_1, batch_y_true_2, batch_gt_box0, batch_gt_box1,
        #                batch_gt_box2):
        #     yolo_output = model_ms(images)
        #     loss = loss_fun(yolo_output, batch_y_true_0, batch_y_true_1, batch_y_true_2, batch_gt_box0, batch_gt_box1,
        ...
        # loss_ms_result = train_step_yolo(seed_model_net,
        #                                  loss_fun_ms,
        #                                  seed_optimizer_fun,
        #                                  data_selected,
        #                                  label_selected1,
        ...
        loss,grads = train_step_3(seed_model_net, loss_fun_ms, seed_optimizer_fun, data_selected, label_selected)
