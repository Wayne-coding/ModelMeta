...
from network.nlp.GPT.src.inference import generate
from network.nlp.GPT.src.dataset import create_dataset
from network.nlp.GPT.src.gpt import GPT, EvalNet, GPTWithLoss
...
def get_acc(model, dataset):
    ...
    for data in dataset:
        ...
        logits = model(Tensor(data, mstype.int32)).asnumpy()
...
def get_acc_torch(model, dataset):
    """ calculate accuracy for input dataset """
    total_num = 0
    acc_num = 0
    for data in dataset:
        ...
        logits = model(torch.tensor(data, dtype=torch.int64).to(final_device)).detach().cpu().numpy()
        logits = logits.reshape(len(length), -1)
