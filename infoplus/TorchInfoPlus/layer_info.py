...
try:
    ...
    def is_lazy(param: nn.Parameter) -> bool:  
        del param
        return False
...
class LayerInfo:
    ...
    def __init__(
        ...
        self.name = names.get(str(id(module)), '[]')
    ...
    def calculate_size(
        ...
        if inputs is None:
            ...
            for input_ in inputs:
                if hasattr(input_,"data"):
                    size.append(list(input_.data.size()))
                    elem_bytes += input_.data.element_size()
...
def nested_list_size(inputs: Sequence[Any] | torch.Tensor):
